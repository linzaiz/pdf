...
for page in pdf.pages[3:4]:
    ...
    # page = page.within_bbox((0, 70, 297.5, 841.5))                   # page.crop(()) 是粗略框一下，压线的字也能抓到。
    table = page.extract_table(table_settings=tbl_settings)
    print(table)

'''

pdf = pdfplumber.open(r"C:\tmp\2019津文.pdf",)  # 加密的pdf报错 Unable ... DLL
...
for page in pdf.pages[9:10]:
    ...
    table12 = page.extract_table(table_settings=tbl_settings_12)    
    print(table12)

    df12 = pd.DataFrame(table12, columns=table12[0])
    print(df12.head(60))
