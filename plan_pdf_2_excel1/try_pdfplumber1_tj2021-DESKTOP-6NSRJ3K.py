...
pdf = pdfplumber.open(r"C:\tmp\2021物.pdf")  # 不能传pdfminer的参数, detect_vertical=False
...
tbl_settings_12 = {
        ...
        "explicit_vertical_lines": [36, 57, 180, 219],
        "explicit_horizontal_lines": [],
        "snap_tolerance": 3,      # 偏离n像素内的平行线将被捕捉到相同的水平或垂直位置。
        "join_tolerance": 3,       # 同一直线上的两个线段若相距n像素之内则连成一个线段。
        ...
        "text_x_tolerance": 20,   # 当文本策略搜索单词时，它将期望每个单词中的各个字母横向相差不超过text_tolerance像素。
        "text_y_tolerance": 30,  #30造成上下读,类古文排版。25以下没影响。
        "intersection_tolerance": 3,   # 当尝试将线段连结成单元格时，正交边缘必须在intersection_tolerance像素内才认为是相交（单元格的边角）。
        #"intersection_x_tolerance": None,
        #"intersection_y_tolerance": None,
    }
